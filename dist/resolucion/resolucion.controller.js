"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ResolucionController = void 0;
const common_1 = require("@nestjs/common");
const commons_controller_1 = require("../commons/commons.controller");
const resolucion_service_1 = require("./resolucion.service");
const update_resolucion_dto_1 = require("../auth/dto/update-resolucion.dto");
const auth_guard_1 = require("../auth/auth.guard");
let ResolucionController = class ResolucionController extends commons_controller_1.BaseController {
    constructor(resolucionService) {
        super();
        this.resolucionService = resolucionService;
    }
    getService() {
        return this.resolucionService;
    }
    async actualizarResolucion(dto) {
        await this.resolucionService.actualizarResolucion(dto);
        return { message: 'Resolución actualizada correctamente' };
    }
};
exports.ResolucionController = ResolucionController;
__decorate([
    (0, common_1.Patch)('actualizar'),
    (0, common_1.UseGuards)(auth_guard_1.AuthGuard),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [update_resolucion_dto_1.UpdateResolucionDto]),
    __metadata("design:returntype", Promise)
], ResolucionController.prototype, "actualizarResolucion", null);
exports.ResolucionController = ResolucionController = __decorate([
    (0, common_1.Controller)('resolucion'),
    __metadata("design:paramtypes", [resolucion_service_1.ResolucionService])
], ResolucionController);
//# sourceMappingURL=resolucion.controller.js.map